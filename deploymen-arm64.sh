...
# install this
sudo apt install python3-pip python3-dev libpq-dev make nginx curl neovim unzip python3.12-venv docker.io docker-compose postgresql-client-common postgresql-client
...
eval `ssh-agent -s` && ssh-add ~/.ssh/fullyfeatured
